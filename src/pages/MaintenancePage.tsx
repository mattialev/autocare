...
export const MaintenancePage = () => {
  ...
  return (
    <>
      ...
      {tab === 'overview' && <section className="card-grid">{deadlines.map((item) => <article className="card" key={item.id}><h2>{item.title}</h2><p>{formatDate(item.dueDate)}</p><p>{item.dueMileage ? formatKm(item.dueMileage) : 'Km indicativi non impostati'}</p><StatusBadge status={item.status} /></article>)}</section>}
      {tab === 'history' && (records.length ? <section className="table-card"><table><thead><tr><th>Intervento</th><th>Data</th><th>Km</th><th>Officina</th><th>Costo</th><th /></tr></thead><tbody>{records.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><span>{item.typeName}</span></td><td>{formatDate(item.performedAt)}</td><td>{formatKm(item.mileage)}</td><td>{item.workshop || '-'}</td><td>{formatCurrency(item.cost)}</td><td><button className="icon-button" onClick={() => setDeleteId(item.id)} aria-label="Elimina"><Trash2 size={17} /></button></td></tr>)}</tbody></table></section> : <EmptyState title="Nessun intervento registrato." action={<button className="button button-primary" onClick={() => setOpen(true)}>Aggiungi il primo intervento</button>} />)}
      {tab === 'plans' && <section className="table-card"><table><thead><tr><th>Intervento</th><th>Ultima data</th><th>Intervallo</th><th>Prossima scadenza</th></tr></thead><tbody>{records.filter((item) => item.isRecurring).map((item) => <tr key={item.id}><td>{item.title}</td><td>{formatDate(item.performedAt)}</td><td>{item.intervalMonths ? `${item.intervalMonths} mesi` : '-'} {item.intervalKm ? ` / ${formatKm(item.intervalKm)}` : ''}</td><td>{formatDate(item.nextDueDate)}</td></tr>)}</tbody></table></section>}
